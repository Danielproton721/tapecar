...
const API_BASE = "https://api.conta.paybeehive.com.br/v1";
...
function authHeader(secretKey) {
  ...
  // (A doc tem uma seção que diz "Bearer" — está ERRADA; testado: só Basic
  //  autentica. O exemplo de curl da doc, com Basic, é o correto.)
...
/**
 * Busca uma transação por id (GET /transactions/{id}). Usado pelo webhook pra
 * CONFIRMAR o pagamento na fonte antes de agir — não confia no corpo do webhook,
 * que qualquer um poderia forjar.
 */
export async function getBeehiveTransaction(id) {
  const secretKey = process.env.PAYMENT_SECRET_KEY || "";
  if (!secretKey) return { ok: false, status: 501, data: {} };

  let res;
  try {
    res = await fetch(`${apiBase()}/transactions/${encodeURIComponent(id)}`, {
      headers: { Authorization: authHeader(secretKey), "Content-Type": "application/json" },
      cache: "no-store",
    });
  } catch (e) {
    return { ok: false, status: 502, data: { message: e.message } };
  }
  const data = await res.json().catch(() => ({}));
  return { ok: res.ok, status: res.status, data };
}
